...
| 能执行本地命令的助手或编辑工具 | 读取 `SKILL.md` 后运行CLI |
| 支持HTTP的工作流编排平台、内部业务系统和自建后端 | 启动HTTP服务并发送JSON |
| 桌面应用、编辑器扩展和长期运行的集成程序 | HTTP或NDJSON长期子进程 |
...
一键测试当前执行131项，覆盖中文数字年限与年月排除、跨行必须/优先作用域、裸斜杠待确认、开放薪资区间与薪资月份范围、真实日期比较、技能且/或及顿号分组、字段缺失与明确为空、未知硬条件、公平性分数不变性、补证脱敏、HTTP结构错误、类别数据门控、四类角色页面、80项指标、接口安全、新旧两条Markdown导出链路的标签/链接/图片/前缀注入防护和Excel动态重载。

## 安全与合规

- 受保护属性先在原始资料中审计，再从评分副本中排除；警告可以变化，但这些内容不能改变匹配分。
- 简历和岗位说明都按不可信数据处理，其中的命令、链接和提示内容不会被执行。
- 角色报告和兼容版 `scripts/io_tools/export/export_markdown.py` 都复用同一组Markdown安全渲染函数，把ID、摘要、条件、指标名、问题、表格、列表及计划中的动态内容统一转为纯文本；输入中的HTML标签、图片、链接语法、反斜杠、换行和 `= + - @` 前缀不会变成新的报告结构。
...
Skill目录名、作品名称和 `SKILL.md` frontmatter中的名称都应保持为：

```text
match-talent-opportunities
```

提交文件使用根目录中的 `match-talent-opportunities-final.zip`。打开ZIP后第一层应直接看到 `SKILL.md`、`README.md`、`LICENSE.md`、`scripts/`、`references/`、`assets/` 和 `reference_indicators/`，不能再套项目文件夹。从比赛赛题页面上传ZIP，由比赛流程同步到SkillHub。
