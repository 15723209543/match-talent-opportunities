...
def build_story(styles, font_name: str, bold_name: str):
    ...
    story.extend([
        ...
        styled_table([
            ...
            [p("4.2.0", styles["center"]), p("80项", styles["center"]), p("4类", styles["center"]), p("131项", styles["center"])],
    ...
    story.extend([
        ...
        p("本轮检查把复检中的严重场景直接写入回归测试，包括中文数字年限与年月排除、跨行必须/优先作用域、斜杠歧义、顿号AND/OR、Go词边界、办公方式否定、开放薪资与薪资月数范围、公平性隔离、字段来源、证据脱敏、新旧Markdown导出链路的动态字段转义和接口类型校验。", styles["body"]),
        styled_table([
            ["检查项", "结果", "说明"],
            ["自动测试", "131项全部通过", "测试运行器自动发现全部test_*.py"],
