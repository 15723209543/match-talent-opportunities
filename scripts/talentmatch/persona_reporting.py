...
def _markdown_table(headers: List[str], rows: List[List[Any]]) -> str:
    ...
    def clean(value: Any) -> str:
        return str(value if value is not None else "待补").replace("|", "\\|").replace("\n", " ")
...
def _question_markdown(questions: Iterable[Dict[str, Any]]) -> str:
    ...
    for index, item in enumerate(values, 1):
        blocks.append(
            f'{index}. **{item.get("focus", "核验项")}**：{item.get("question", "")}\n'
            f'   - 评分锚点：{item.get("score_anchor", "")}\n'
            f'   - 建议材料：{item.get("evidence_to_request", "")}\n'
            f'   - 追问条件：{item.get("follow_up_if", "回答边界或证据不清时继续追问。")}'
        )
    return "\n".join(blocks)
...
def _chart_markdown(charts: List[Dict[str, str]], report_path: Path | None) -> str:
    ...
    for chart in charts:
        ...
        if report_path:
            ...
            target_text = target.as_posix()
        lines.append(f'![{chart["alt"]}]({target_text})')
    return "\n\n".join(lines)
...
def render_markdown(result: Dict[str, Any], persona: str, charts: List[Dict[str, str]] | None = None, report_path: Path | None = None) -> str:
    ...
    sections = [
        f'# {profile["title"]}',
        f'> 候选人 `{meta.get("candidate_id")}` × 岗位 `{meta.get("job_id")}`。{payload["decision_scope"]}',
        "## 先看结论",
    ...
    if persona == "hr":
        sections.extend([
            ...
            "## HR操作建议", "\n".join(f"- {item}" for item in recommendations.get("recruiter_actions", [])),
        ])
    elif persona == "candidate":
        sections.extend([
            ...
            "## 简历改进", "\n".join(f"- {item}" for item in recommendations.get("resume_actions", [])),
            ...
            "## 30/60/90天提升建议", "```json\n" + json.dumps(recommendations.get("development_plan", {}), ensure_ascii=False, indent=2) + "\n```",
            "## 真实陈述提醒", payload.get("do_not_overclaim", ""),
        ])
    elif persona == "interviewer":
        sections.extend([
            ...
            "## 追问条件", "\n".join(f"- {item}" for item in payload.get("follow_up_conditions", [])) or "- 暂无预设追问。",
            ...
            "## 记录规则", payload.get("recording_rule", ""),
            "## 不应询问的内容", "\n".join(f"- {item}" for item in payload.get("prohibited_topics", [])),
        ])
    else:
        sections.extend([
            ...
            "## 成果验收依据", "\n".join(f"- {item}" for item in payload.get("success_evidence", [])),
        ])

    completeness = analytics.get("data_completeness", {})
    sections.extend([
        "## 数据质量与适用边界",
        f'- 已知指标：{completeness.get("known_metrics", 0)}；待补指标：{completeness.get("unknown_metrics", 0)}；不适用指标：{completeness.get("not_applicable_metrics", 0)}。',
    ...
    sections.extend([
        "## 方法与审计信息",
        f'- 有效指标：{result.get("method", {}).get("indicator_count", 0)}项，系数来自可编辑Excel。',
        f'- Excel SHA-256：`{result.get("method", {}).get("indicator_workbook_sha256", "")}`。',
