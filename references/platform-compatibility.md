...
| 能执行本地命令的助手或编辑工具 | 读取 `SKILL.md` 后调用CLI |
| 支持HTTP的工作流编排平台、内部业务系统和自建后端 | `scripts/interfaces/http/http_server.py` |
| 桌面应用、编辑器扩展和长期运行的集成程序 | HTTP或 `scripts/interfaces/stdio/json_stdio.py` |
